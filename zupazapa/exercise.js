...
exercise.one = function(a, b){
    ...
    // ----------------------------------------
    return {'sum': a+b}; //to match the output format being required
...
exercise.three = function(str){
    ...
    // ----------------------------------------
    var newString = '';
    for(var i=0; i<str.length-1; i++){ //the -1 allows us to append '_' only for the first 4 string items
        newString += str[i] + '_';
    }
    newString += str[str.length-1];//append the last string position of 'apple', which is e
    return newString;


    //return str.split("").join("_"); //another way to solve the problem with split and join
...
exercise.four = function(arr){
    ...
    // ----------------------------------------
        ...
        //     root = Math.square(arr[i]); //not a function, 
        //     sqArr.push(root);
        // }
        // return sqArr;

var newArr = arr.map(function(int){return int*int;});
return newArr;
...
exercise.five = function(obj){
    ...
    // ----------------------------------------
    var name = obj.first_name + ' ' + obj.last_name;//objects have attribute defined as 'first_name' and another as 'last_name'
    return name;
 //var name = obj['first_name'] + ' ' + obj['last_name']; //this also works
...
//return arr.filter(function(i) {return i%2 === 0; })  // there's a .length in here
...
exercise.seven = function(n){
    ...
    newArr = [];
    for (var i=0; i<n; i++){
        if (i%2===0) newArr.push(0);
        else newArr.push(1); //this is how you can build an array from scratch
    }
    return newArr;
};
...
exercise.nine = function(func){
    ...
    return func()*func(); //the only way to do this
};
