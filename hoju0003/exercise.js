...
exercise.three = function(str){
    ...
    return newString+=str[str.length-1];
...
exercise.four = function(arr){
    ...
    var newArray= arr.map(function(item,i,wholething){
        return item*item;
    }) ;
    return newArray;
...
exercise.five = function(obj){
    ...
    var newString= obj.first_name +' '+ obj.last_name;
    return newString;
...
exercise.six = function(arr){
    ...
    arr.forEach(function(item,i,wholething){
        if (item % 2 == 0 ){
            count = count + 1;
            }
        });
...
exercise.eight = function(str){
    ...
    return newArray.reduce(function(prev,current){
        
        return prev=Number(prev)+Number(current)//first prev initialise as 0;
    });
